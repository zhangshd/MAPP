#!/usr/bin/env python
"""
MAPP vs MAPPPure+IAST Speed Benchmark (PSA Grid Simulation)
...
2. MAPPPure+IAST: Pure component prediction (once) + Fitting (once) + IAST calculation (loop over all conditions).
"""
...
# ============================================================================
# MAPPPure + IAST Benchmark Module
# ============================================================================
...
def benchmark_ml_iast_grid(cif_list, pure_model_dir, output_dir, ps_grid, ys_grid, n_runs=1):
    """
    Benchmark MAPPPure + IAST on a dense grid.
...
def plot_benchmark_results(mapp_res, iast_res, output_dir, n_mofs, n_points):
    """Plot comparison between MAPP and ML-Pure+IAST."""
    ...
    colors = [cmap(i / 5) for i in range(2)]
    
    # 1. Bar Chart - Total Time
    methods = ['MAPP', 'MAPPPure+IAST']
    times = [mapp_res["timings"]["total"], iast_res["timings"]["total"]]
    
    bars = ax1.bar(methods, times, color=colors, width=0.6, alpha=0.8)
    ax1.set_ylabel('Total Time (s)', fontsize=14)
    ax1.set_title(f'Time to Process {n_mofs} MOFs × {n_points} Points\nTotal Queries: {n_mofs*n_points}', 
    ...
    # Color palette for stacked bars (matching other scripts' style)
    stack_colors = [cmap(i / 10) for i in range(10)]
    
    # Plot MAPP stack
    bottom = 0
    for idx, (t, l) in enumerate(zip(mapp_comps, mapp_labels)):
        ax2.bar(0, t, bottom=bottom, label=f"MAPP: {l}", width=0.5, color=stack_colors[idx], alpha=0.8)
        bottom += t
        
    # Plot IAST stack
    bottom = 0
    for idx, (t, l) in enumerate(zip(iast_comps, iast_labels)):
        ax2.bar(1, t, bottom=bottom, label=f"IAST: {l}", width=0.5, color=stack_colors[idx+4], alpha=0.8)
        # Add text for IAST loop if significant
        if l == "IAST Loop" and t > 5:
             ax2.text(1, bottom + t/2, "IAST Loop", ha='center', va='center', 
                     color='white', fontweight='bold', fontsize=12)
        bottom += t
        
    ax2.set_xticks([0, 1])
    ax2.set_xticklabels(['MAPP', 'MAPPPure+IAST'], fontsize=14)
    ax2.set_ylabel('Time (s)', fontsize=14)
    ax2.set_title('Time Breakdown', fontsize=15, pad=10, fontweight='bold')
    ax2.tick_params(axis='both', labelsize=14)
    ax2.legend(loc='upper left', bbox_to_anchor=(1, 1), fontsize=12, framealpha=0.8)
    
    plt.tight_layout()
    plt.savefig(output_dir / "grid_benchmark_comparison.png", dpi=300, bbox_inches='tight')
    print(f"Plot saved to {output_dir / 'grid_benchmark_comparison.png'}")


def main():
    # Extract default paths from config
    default_mixture_model = config["models"]["MAPP_GMOF"]
    default_pure_model = config["models"]["MAPPPure"]
    default_cif_dir = config["data"]["ddmof_cifs"]
    default_output_dir = PROJECT_ROOT / config["output"]["results"] / "benchmark_psa"
    ...
    parser.add_argument('--mixture_model', type=str, default=default_mixture_model, 
                       help=f'Mixture model path (default from config: {default_mixture_model})')
    parser.add_argument('--pure_model', type=str, default=default_pure_model,
                       help=f'Pure component model path (default from config: {default_pure_model})')
    parser.add_argument('--cif_dir', type=str, default=default_cif_dir,
                       help=f'CIF directory path (default from config: {default_cif_dir})')
    parser.add_argument('--output_dir', type=str, default=str(default_output_dir),
                       help=f'Output directory (default from config: {default_output_dir})')
    ...
    mapp_res = benchmark_mapp_grid(cif_list, args.mixture_model, output_dir, P, Y, args.n_runs)
    print(f"  > MAPP Total Time: {mapp_res['timings']['total']:.2f}s")
    
    print("\nRunning MAPPPure + IAST Grid ...")
    iast_res = benchmark_ml_iast_grid(cif_list, args.pure_model, output_dir, P, Y, args.n_runs)
